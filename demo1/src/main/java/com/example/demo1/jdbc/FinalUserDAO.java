...
public class FinalUserDAO {
    ...
    private static final String SQL_INSERT1 = "insert into final_users(id, user_name, username, user_password, email, balance) value(?,?,?,?,?,?)";
    ...
    public int insertId(FinalUser finalUser){
        Connection con = null;
        PreparedStatement ps = null;
        int records = 0;
        try {
            ...
            ps.setString(2,finalUser.getName());
            ps.setString(3,finalUser.getUsername());
            ps.setString(4,finalUser.getPassword());
            ps.setString(5,finalUser.getEmail());
            ps.setDouble(6,finalUser.getBalance());
            records = ps.executeUpdate();
