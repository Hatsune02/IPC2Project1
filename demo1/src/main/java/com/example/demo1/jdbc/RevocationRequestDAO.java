...
public class RevocationRequestDAO {
    ...
    private static final String SQL_INSERT1 = "insert into revocation_requests(id,final_user,state,detail) value(?,?,?,?)";
    ...
    public int insertId(RevocationRequest revocationRequest) {
        Connection con = null;
        PreparedStatement ps = null;
        int records = 0;
        try {
            ...
            ps.setInt(2, revocationRequest.getFinalUserId());
            ps.setString(3, revocationRequest.getState());
            ps.setString(4, revocationRequest.getDetail());
            records = ps.executeUpdate();
