...
public class CarrierDAO {
    ...
    private static final String SQL_INSERT1 = "insert into carriers(id,carrier_name, username, carrier_password, email) value(?,?,?,?,?)";
    ...
    private static final String SQL_LESS_TRANSPORTS_B = "select carriers.id, carriers.username, count(transport_between_libraries.carrier) as transports from carriers\n" +
            "left join transport_between_libraries on transport_between_libraries.carrier = carriers.id\n" +
            "group by id order by transports limit 1;";
    private static final String SQL_LESS_TRANSPORTS_U = "select carriers.id, carriers.username, count(transport_users.carrier) as transports from carriers\n" +
            "left join transport_users on transport_users.carrier = carriers.id\n" +
            "group by id order by transports limit 1;";
    ...
    public int insertId(Carrier carrier){
        Connection con = null;
        PreparedStatement ps = null;
        int records = 0;
        try {
            ...
            ps.setString(3,carrier.getUsername());
            ps.setString(4,carrier.getPassword());
            ps.setString(5,carrier.getEmail());
            records = ps.executeUpdate();
    ...
    public int carrierLessTransportsB(){
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        int records = 0;
        try {
            con = getConnection();
            ps = con.prepareStatement(SQL_LESS_TRANSPORTS_B);
            rs = ps.executeQuery();
            while(rs.next()){
                records = rs.getInt("carriers.id");
            }
    ...
    public int carrierLessTransportsU(){
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        int records = 0;
        try {
            con = getConnection();
            ps = con.prepareStatement(SQL_LESS_TRANSPORTS_U);
            rs = ps.executeQuery();
            while(rs.next()){
                records = rs.getInt("carriers.id");
            }
